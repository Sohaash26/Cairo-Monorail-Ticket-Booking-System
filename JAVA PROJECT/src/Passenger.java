...
public class Passenger extends Person implements Serializable {
	...
	public ArrayList <Passenger> Passengers = new ArrayList<>(); // Collection of passengers
	public static int countPassengers = 0; // to count passengers 
	public ArrayList <Ticket> Booked_Tickets = new ArrayList<>(); // Collection of tickets
	...
	public void viewInfo() // Overridden method from person
	{
		for(int i =0; i<Passengers.size();i++)
		{
			...
			System.out.println("Name: " + Passengers.get(i).getName()); // name saved from arraylist
			System.out.println("Age: " + Passengers.get(i).getAge()); // age saved from arraylist
			System.out.println("ID: " + Passengers.get(i).getID()); // id saved from arraylist
			System.out.println("E-mail: " + Passengers.get(i).getAcc().getEmail()); // email saved from arraylist
			System.out.println("Password: " + Passengers.get(i).getAcc().getPassword()); // password saved from arraylist
	...
	public void createAcc()
	{
		...
		{	
			if(enteredID == Passengers.get(i).getID())
			{
				...
				viewInfo();
                                try // first exception (not user defined)
                                {
                                   /* FileOutputStream Outputfile = new FileOutputStream("C:\\Users\\Muhammad\\Desktop\\JAVA PROJECT\\src\\Accounts.txt");
                                    ObjectOutputStream output = new ObjectOutputStream(Outputfile);
                                    
                                    for(Passenger x : Passengers)
                                    {
                                         output.writeObject(x.getAcc());  
                                    } */
                                    
                                    Formatter z = new Formatter("C:\\Users\\Muhammad\\Desktop\\JAVA PROJECT\\src\\Accounts.txt");
                                    z.format("Passenger # %d\n" + "Name: %s\n" + "Age: %d\n" + "ID: %d\n" +"Email: %s\n" + "Password: %s\n"
                                            + "--------------------------------" , 
                                            Passenger.countPassengers ,Passengers.get(i).getName()
                                            , Passengers.get(i).getAge() , Passengers.get(i).getID() ,
                                            Passengers.get(i).getAcc().getEmail() , Passengers.get(i).getAcc().getPassword());
                                    z.close();
                                    
                                    //output.close();
                                    
                                   /* FileInputStream inputFile = new FileInputStream("C:\\Users\\Muhammad\\Desktop\\JAVA PROJECT\\src\\Accounts.txt");
                                    ObjectInputStream Objinput = new ObjectInputStream(inputFile);
                                    
                                    System.out.println("Email: " + Objinput.readObject());
                                    System.out.println("Password: " + Objinput.readObject());
                                    Objinput.close(); */
			...
			else
			{
                ...
                {
                    System.out.println("ID should not be String");
                }
	...
	public void manageAcc()
	{
		...
		for(int i =0; i<Passengers.size();i++)
		{
			if(enteredID == Passengers.get(i).getID())
			{
				...
				if(Passengers.get(i).getAcc().getEmail() == null && Passengers.get(i).getAcc().getPassword() == null)
				{
    ...
    public String toString() {
        return "Passenger{" + "Passengers=" + Passengers + ", Booked_Tickets=" + Booked_Tickets + '}';
    }
